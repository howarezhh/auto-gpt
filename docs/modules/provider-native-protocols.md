...
- 提供商可配置 `native_endpoint_path` 作为 Gemini/Claude 原生协议的自定义接口路径模板；为空时使用官方默认路径。模板支持 `{model}`、`{raw_model}`、`{action}`，其中 Gemini 默认非流式 action 为 `generateContent`，流式 action 为 `streamGenerateContent?alt=sse`。
...
- 提供商新增/编辑、模型挂载矩阵和模型管理页面必须展示 `protocol_type` 与 `model_group`；模型挂载矩阵筛选区必须支持按 `model_group` 过滤挂载记录。
- 提供商新增/编辑表单必须展示 `protocol_type` 与 `native_endpoint_path`；选择 Google Gemini 或 Anthropic/Claude 类型时，前端应自动选择对应原生协议并给出路径模板占位，但管理员可以人工修改。
- 协议下拉应包含 `双协议`、`Chat`、`Responses`、`Gemini`、`Claude`。
