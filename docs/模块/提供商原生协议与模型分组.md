...
- 模型映射选择目标模型时必须同时考虑请求端点、请求能力和原生协议约束。不得只按映射配置顺序选择目标模型后再让后续路由失败；不可路由目标必须在映射 trace 中记录原因和目标路由诊断。
...
- 若模型挂载存在 `upstream_model_name`，可用性检测、内容防护探针、端点协议判断、价格目录匹配和代理转发必须统一使用该上游真实模型名；展示、权限和日志中的本项目模型别名仍保留为 `model_name`。当前内部探针和兼容字段仍可能使用 `health_probe` / `health_status` 命名。
...
- 后台可用性检测、单模型测试、提供商挂载模型测试、内容防护可信检测和端点协议检测的所有结果列表，都必须在每条检测结果旁提供“详情”按钮；详情弹层必须以原始 JSON 展示本次检测结果对象，并在存在 `raw_provider_response` 时包含真实上游响应体或流式首块，同时提供一键复制 JSON。详情 JSON 禁止包含请求头、API Key、Token、密码等敏感信息。
