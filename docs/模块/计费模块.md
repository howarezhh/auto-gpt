...
- `UserPortalService` 负责后台用户列表的额度用量汇总，展示每个用户今日、月度、累计请求、可计费 token、费用、可用余额占比和风险状态。
...
5. 请求完成后，若日志已包含上游官方 `usage` 或 Gemini、Claude 等原生协议转换出的官方 usage，必须在日志落库事务中立即调用 `BillingService.finalize_request_log_billing()` 完成成本计算、余额扣减和账单记录写入；后台 Token finalize 只作为漏记、异常中断或历史数据修正的补偿兜底。
6. `BillingService.compute_log_cost()` 只能以官方 usage 产生的准确 token 作为生产结算事实，根据真实 token、缓存 token、费用组件和汇率快照计算原币种成本与账户币种成本。本地 tokenizer、tiktoken 或请求/响应文本推算出的 token 只能标记为 `estimated` 用于观测和排查，禁止作为最终扣费依据。
7. 请求日志的 `billable` 口径决定是否进入计费；`success` 只表示请求结果，不再直接作为计费开关。完整成功请求默认可计费；失败请求只有在已产生上游响应、已输出内容、已产生 token 或内容安全在响应过程中中断时才可计费，其中内容安全中断后的可计费日志必须记录 `billable_reason=blocked_after_output`。
8. Token 回填允许对价格快照全为 0 的请求走快速不扣费路径，但必须先用当前模型价格预演确认费用仍为 0，且 token 来源必须是官方 usage；禁止仅凭旧日志快照或估算 token 把可计费请求终态写成 `no_charge`。
9. 已终态 `no_charge` 的日志默认不重复结算；仅当该日志可计费、已有官方 usage token，且按当前模型价格预演会产生正费用时，允许重新打开结算并修正为 `billed`。
10. `sync_request_billing()` 写入 API Key 账单和用户账户账单，并更新账户余额。重复处理同一请求必须幂等。
