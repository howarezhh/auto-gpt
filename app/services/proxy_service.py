...
from app.services.routing import RouteCapabilitySet, RoutePolicyContext, RouteRequest, RouteRequestKind, RoutingService
...
from app.utils.content_guard_config import CONTENT_GUARD_STREAM_BUFFER_MAX_BYTES_LIMIT
...
class ContentGuardBlockedError(Exception):
    ...
    def __init__(
        ...
        *,
        ...
        status_code: int = ContentRuntimeGuardService.CONTENT_GUARD_ERROR_STATUS_CODE,
...
class FixedSuccessResponseDetectedError(Exception):
    """表示上游以 HTTP 200 返回固定失败文本，需按可重试上游故障处理。"""

    def __init__(self, *, detection: dict[str, Any], status_code: int = status.HTTP_502_BAD_GATEWAY) -> None:
        super().__init__(ProviderHealthStateService.FIXED_SUCCESS_RESPONSE_ERROR_CODE)
        self.detection = detection
        self.status_code = status_code
        self.detail = {
            "message": "上游返回了高度相似的固定失败文本，已按伪成功响应故障切换候选",
            "code": ProviderHealthStateService.FIXED_SUCCESS_RESPONSE_ERROR_CODE,
            "detection": detection,
        }
...
class RouteExhaustedRetrySignal(Exception):
    ...
    def __init__(
        ...
        *,
        ...
        route_retry_failed_candidate_keys: set[tuple[int, int]] | None = None,
    ) -> None:
        ...
        self.route_retry_failed_candidate_keys = set(route_retry_failed_candidate_keys or set())
...
class ProxyService:
    ...
    MODEL_CATALOG_LIMIT_LOCK_LIMIT = 2048
    ROUTE_RETRY_TRACE_LIMIT = 100
    ROUTE_OUTAGE_CACHE_PREFIX = "route-outage"
    ROUTE_OUTAGE_CACHE_MAX_TTL_SECONDS = 300
    FIXED_SUCCESS_RESPONSE_CONFIRMATION_RETRY_DELAYS_SECONDS = (2, 5, 10)
    _provider_success_update_at: dict[tuple[int, int], float] = {}
    _requests_session_local = threading.local()
    _requests_connection_semaphore: threading.BoundedSemaphore | None = None
    _requests_connection_semaphore_fingerprint: tuple[int] | None = None
    _requests_connection_semaphore_guard = threading.Lock()
    ...
    async def _run_db_write(operation, *args, db: Session | None = None, **kwargs):
        ...
        if db is not None:
            try:
                result = await run_in_threadpool(operation, db, *args, **kwargs)
                db.commit()
                return result
            except Exception:
                db.rollback()
    ...
    def _model_catalog_limit_lock(cache_key: str) -> threading.Lock:
        with ProxyService._model_catalog_limit_locks_guard:
            ...
            if lock is None:
                if len(ProxyService._model_catalog_limit_locks) >= ProxyService.MODEL_CATALOG_LIMIT_LOCK_LIMIT:
                    ProxyService._model_catalog_limit_locks.clear()
                lock = threading.Lock()
    ...
    async def _reject_by_request_token_limit_async(
        ...
        *,
        ...
        await ProxyService._run_db_write(
            ...
            success=False,
            status_code=status_code,
            duration_ms=ProxyService._elapsed_ms_since(request_started_at),
    ...
    @staticmethod
    async def _precheck_owner_balance_for_candidate(
        db: Session,
        *,
        api_client_auth: ApiClientAuthContext | None,
        provider_model: ProviderModel,
        ...
        if available_balance >= estimated_cost:
            try:
                await BillingReservationService.acquire(
                    user_account_id=int(owner_user.id),
                    reservation_id=ProxyService._billing_reservation_id(trace_id),
                    estimated_amount=estimated_cost,
                    available_balance=available_balance,
                    ttl_seconds=int(getattr(get_settings(), "concurrency_lease_ttl_seconds", 900) or 900),
                )
            except BillingReservationError as exc:
                return {
                    "message": exc.args[0] if exc.args else "账户可用余额不足以覆盖当前在途请求和本次请求的预估费用",
                    "code": exc.code,
                    "estimated_cost": BillingService.to_float(estimated_cost),
                    "available_balance": BillingService.to_float(available_balance),
                    "estimated_input_tokens": estimated_input_tokens,
                    "estimated_output_tokens": estimated_output_tokens,
                    "active_reserved_amount": exc.active_reserved_amount,
                }
            except BillingReservationUnavailableError as exc:
                raise ProxyService._capacity_service_unavailable_http_exception(
                    ProviderCapacityUnavailableError(str(exc))
                ) from exc
            return None
        return {
    ...
    @staticmethod
    def _billing_reservation_id(trace_id: str | None) -> str:
        return str(trace_id or uuid4().hex)

    @staticmethod
    def _release_owner_balance_reservation(api_client_auth: ApiClientAuthContext | None, *, trace_id: str | None) -> None:
        if api_client_auth is None:
            return
        owner_user = getattr(api_client_auth.api_client_key, "owner_user", None)
        owner_user_id = getattr(owner_user, "id", None)
        if owner_user_id is None:
            owner_user_id = getattr(api_client_auth.api_client_key, "owner_user_id", None)
        BillingReservationService.release(
            user_account_id=int(owner_user_id) if owner_user_id is not None else None,
            reservation_id=ProxyService._billing_reservation_id(trace_id) if trace_id else None,
        )
    ...
    @staticmethod
    def _mark_fixed_success_response_failure(
        db: Session,
        provider: Provider,
        provider_model: ProviderModel,
        latency_ms: int,
        ...
        now = now_beijing()
        ProviderHealthStateService.record_fixed_success_response_failure(
            provider,
            provider_model,
    ...
    @staticmethod
    def _mark_fixed_success_response_failure_by_id(
        db: Session,
        provider_id: int,
        provider_model_id: int,
        latency_ms: int,
        ...
        if provider is None or provider_model is None:
            return
        ProxyService._mark_fixed_success_response_failure(db, provider, provider_model, latency_ms, detection)

    @staticmethod
    async def _mark_fixed_success_response_failure_async(
        provider: Provider,
        provider_model: ProviderModel,
        latency_ms: int,
        detection: dict[str, Any],
        *,
        ...
        if db is not None:
            await run_in_threadpool(
                ProxyService._mark_fixed_success_response_failure,
                db,
                provider,
                provider_model,
                latency_ms,
            ...
            return
        await ProxyService._run_db_write(
            ProxyService._mark_fixed_success_response_failure_by_id,
            provider.id,
            provider_model.id,
            latency_ms,
            detection,
    ...
    @staticmethod
    async def _confirm_fixed_success_response_detection(
        provider: Provider,
        provider_model: ProviderModel,
        detection: dict[str, Any],
        *,
        ...
        retry_delays = ProxyService.FIXED_SUCCESS_RESPONSE_CONFIRMATION_RETRY_DELAYS_SECONDS
        started_at = time.perf_counter()

        def _build_decision(
            *,
            confirmed_normal: bool,
            should_mark_unhealthy: bool,
            reason: str,
            probe_result: dict[str, Any] | None = None,
        ) -> dict[str, Any]:
            decision = {
                "confirmed_normal": confirmed_normal,
                "should_mark_unhealthy": should_mark_unhealthy,
                "reason": reason,
                "endpoint_path": endpoint_path,
                "attempts": attempts,
                "probe_result": ProxyService._compact_fixed_success_confirmation_result(probe_result),
            }
            detection["fixed_answer_confirmation"] = decision
            if trace is not None:
                trace.append(
                    ProxyService._build_trace_item(
                        provider,
                        provider_model,
                        "fixed_answer_probe_confirmed" if confirmed_normal else "fixed_answer_probe_failed",
                        int((time.perf_counter() - started_at) * 1000),
                        status_code=200 if confirmed_normal else int((probe_result or {}).get("status_code") or 502),
                        error=None if confirmed_normal else reason,
                        extra={
                            "fixed_answer_confirmation": decision,
        ...
        if not endpoint_path:
            return _build_decision(
                ...
                reason="fixed_answer_probe_endpoint_unavailable",
            )
        ...
        for attempt_index, delay_seconds in enumerate((0, *retry_delays), start=1):
            ...
            probe_result = await ContentGuardProbeService.probe_fixed_answer(provider, provider_model, endpoint_path=endpoint_path)
            last_result = probe_result if isinstance(probe_result, dict) else {"message": str(probe_result)}
            rate_limited = ProbeRateLimitService.is_rate_limited_result(last_result)
            attempt = {
                "attempt_index": attempt_index,
                "delay_before_seconds": delay_seconds,
                ...
                "error_code": last_result.get("error_code") or last_result.get("support_mode"),
                "message": last_result.get("message"),
                "support_label": last_result.get("support_label"),
            }
            attempts.append(attempt)
            if attempt["success"]:
                FixedSuccessResponseService.clear_samples(provider.id, provider_model.id)
                return _build_decision(
                    confirmed_normal=True,
                    should_mark_unhealthy=False,
                    reason="fixed_answer_probe_passed",
            ...
            if not rate_limited:
                return _build_decision(
                    confirmed_normal=False,
                    should_mark_unhealthy=True,
                    reason="fixed_answer_probe_failed",
                    probe_result=last_result,
                )
        return _build_decision(
            confirmed_normal=False,
            should_mark_unhealthy=True,
            reason="fixed_answer_probe_rate_limited",
            probe_result=last_result,
        )

    @staticmethod
    def _compact_fixed_success_confirmation_result(result: dict[str, Any] | None) -> dict[str, Any] | None:
        ...
        if "trace" in result:
            compact["trace"] = result.get("trace")
        if "content_guard" in result:
            compact["content_guard"] = result.get("content_guard")
        return compact
    ...
    @staticmethod
    def _upsert_success_log_with_provider_status(db: Session, *args, **kwargs):
        resolved_provider_model_id = kwargs.get("resolved_provider_model_id")
        provider_id = kwargs.get("provider_id")
        latency_ms = kwargs.get("latency_ms")
        if (
            ...
            ProxyService._mark_success_by_id(db, provider_id, resolved_provider_model_id, latency_ms)
        return LogService.upsert_log(db, *args, auto_commit=False, **kwargs)
    ...
    def _content_guard_retry_provider_count(trace: list[dict] | None) -> int:
        ...
        return sum(
            1
            for item in trace
            if isinstance(item, dict) and item.get("result") == "content_integrity_violation"
        )

    @staticmethod
    def _content_guard_log_kwargs_from_trace(
        ...
        *,
        ...
        if isinstance(trace, list):
            for item in reversed(trace):
                if not isinstance(item, dict) or item.get("typed_event") != "request_content_guard":
                    continue
                candidate = item.get("payload")
                if isinstance(candidate, dict):
                    payload = candidate
                    break
        if payload is None and isinstance(detail, dict):
            error = detail.get("error")
            error_detail = error.get("detail") if isinstance(error, dict) else None
            guard = error_detail.get("content_guard") if isinstance(error_detail, dict) else None
            if isinstance(guard, dict):
                payload = {
                    "guard_result": guard.get("result"),
                    "risk_level": guard.get("risk_level"),
                    "matched_categories_json": dumps_json(guard.get("categories")),
                    "matched_rules_json": dumps_json(guard.get("matched_rules")),
                    "reason": guard.get("reason"),
                    "action": guard.get("action"),
                    "excerpt": guard.get("excerpt"),
                    "latency_ms": guard.get("latency_ms"),
                    "buffer_wait_ms": guard.get("buffer_wait_ms"),
                    "final_strategy": guard.get("final_strategy"),
        ...
        if not isinstance(payload, dict):
            return {}
        return {
            "content_guard_result": payload.get("guard_result"),
            "content_guard_risk_level": payload.get("risk_level"),
            "content_guard_categories_json": payload.get("matched_categories_json"),
            "content_guard_reason": payload.get("reason"),
            "content_guard_action": payload.get("action"),
            "content_guard_excerpt": payload.get("excerpt"),
            "content_guard_retry_provider_count": ProxyService._content_guard_retry_provider_count(trace),
            "content_guard_final_strategy": final_strategy or payload.get("final_strategy"),
            "content_guard_confidence": payload.get("confidence"),
            "content_guard_score_delta": payload.get("score_delta"),
        }

    @staticmethod
    def _content_guard_typed_payload(guard_result: ContentGuardResult, *, guard_stage: str, provider: Provider | None = None) -> dict[str, Any]:
        return {
    ...
    @staticmethod
    def _should_append_content_guard_typed_event(guard_result: ContentGuardResult, *, setting: Any, blocked: bool) -> bool:
        if blocked:
            return True
        if guard_result.result != ContentGuardService.RESULT_PASS:
            return True
        if guard_result.risk_level and guard_result.risk_level != "low":
            return True
        if guard_result.final_strategy:
            return True
        return False
    ...
    async def _prefetch_stream_guard_buffer(
        *,
        ...
        timeout_policy: StreamTimeoutPolicy | None = None,
    ) -> tuple[bytes, ContentGuardResult, int | None, bool, asyncio.Task[bytes] | None]:
        buffer = bytearray()
        first_chunk_latency_ms: int | None = None
        stream_ended = False
        prefetch_started = time.perf_counter()
        deadline = None if full_buffer_mode else time.perf_counter() + min(0.5, max(0.0, max_delay_seconds))
        while len(buffer) < limit_bytes:
            ...
            if full_buffer_mode:
                try:
                    if timeout_policy is None:
                        ...
                        timeout_seconds, timeout_code, timeout_message = ProxyService._next_stream_read_timeout(
                            ...
                            stream_started=prefetch_started,
                            timeout_policy=timeout_policy,
                        )
                        if timeout_seconds is None:
                            chunk = await read_task
                        else:
                            chunk = await asyncio.wait_for(read_task, timeout=timeout_seconds)
                ...
                remaining = (deadline or time.perf_counter()) - time.perf_counter()
                if remaining <= 0:
                    read_task.cancel()
                    with suppress(asyncio.CancelledError):
                        await read_task
                ...
                if not done:
                    guard_result = (
                        ...
                        if buffer
                            ...
                            risk_level="low",
                            categories=["stream_prefetch_empty"],
                            reason="流式预检等待窗口超时且未收到上游数据，可能为上游慢首包，转为后续分块检测",
                            action="defer_to_chunk_scan",
                            final_strategy="stream_prefetch_deferred",
                        )
                    )
            ...
            if stop_at_first_event and (b"\n\n" in buffer or b"\r\n\r\n" in buffer):
                break
        if full_buffer_mode and buffer and not stream_ended:
            guard_result = ContentGuardResult(
                result=ContentGuardService.RESULT_BLOCK,
                risk_level="high",
                categories=["stream_full_buffer_incomplete"],
                reason="full_buffer 模式下流式响应未在内容防护缓冲上限内完整结束，已阻断半缓冲透传",
                action="block",
                score_delta=-20,
                confidence=0.9,
        ...
        if not buffer:
            guard_result = ContentGuardResult(
                ...
                categories=["stream_prefetch_empty"],
                reason=(
                    "流式预检期间上游已结束但未返回可检测内容"
                    if stream_ended
                    else "流式预检未收到可检测内容，转为后续分块检测"
                ),
                action="defer_to_chunk_scan",
                final_strategy="stream_prefetch_deferred",
            )
    ...
    @staticmethod
    async def _resolve_pending_stream_guard_prefetch_before_downstream(
        *,
        db: Session | None,
        setting: Any,
        provider: Provider,
        ...
        request_payload: dict[str, Any],
        chunk_iterator: AsyncIterator[bytes],
        pending_read_task: asyncio.Task[bytes],
        started: float,
        limit_bytes: int,
        route_context: RoutePolicyContext | None = None,
    ) -> tuple[bytes, ContentGuardResult, int | None, bool, asyncio.Task[bytes] | None]:
        ...
        read_task: asyncio.Task[bytes] | None = pending_read_task
        while len(buffer) < limit_bytes:
            try:
                ...
                guard_result = ContentGuardResult(
                    ...
                    risk_level="low",
                    categories=["stream_prefetch_empty"],
                    reason="流式预检等待首块期间上游已结束，未返回可检测内容",
                    action="defer_to_chunk_scan",
                    final_strategy="stream_prefetch_deferred",
                )
                guard_result.buffer_wait_ms = max(0, int((time.perf_counter() - stream_started) * 1000))
                return bytes(buffer), guard_result, first_chunk_latency_ms, True, None
            finally:
                read_task = None
            if not chunk:
                continue
        ...
        guard_result.buffer_wait_ms = max(0, int((time.perf_counter() - stream_started) * 1000))
        return bytes(buffer), guard_result, first_chunk_latency_ms, False, None
    ...
    async def _log_content_guard_violation(
        *,
        ...
        await ProxyService._run_db_write(
            ...
            error_type=OpenAIErrorService.classify_error(
                status_code=ContentRuntimeGuardService.CONTENT_GUARD_ERROR_STATUS_CODE,
    ...
    async def forward_json_request(db: Session | None = None, **kwargs) -> tuple[dict[str, Any], Provider, list[dict], int]:
        if db is None:
            with SessionLocal() as scoped_db:
                return await ProxyService.forward_json_request(scoped_db, **kwargs)
        ...
        route_retry_failed_candidate_keys = set(kwargs.pop("route_retry_failed_candidate_keys", set()) or set())
        while True:
            try:
                return await ProxyService._forward_json_request_once(
                    ...
                    **kwargs,
                    ...
                    route_retry_failed_candidate_keys=route_retry_failed_candidate_keys,
                )
                ...
                route_retry_failed_candidate_keys = signal.route_retry_failed_candidate_keys
    @staticmethod
    async def _forward_json_request_once(
        ...
        *,
        ...
        base_log_kwargs = ProxyService._build_request_log_base_kwargs(
            log_type=log_type,
            trace_id=trace_id,
            model_name=model_name,
            requested_model=requested_model_name,
        ...
        await ProxyService._upsert_live_request_log_async(
            db,
            base_log_kwargs=base_log_kwargs,
            trace=trace,
            request_status=LogService.REQUEST_STATUS_REQUESTING,
            message="正在请求提供商",
            attempt_count=route_retry_attempt_count,
        )
        ...
        failed_candidate_keys: set[tuple[int, int]] = set(route_retry_failed_candidate_keys or set())
        if await RoutingService.maybe_wait_for_route_outage(
            trace,
            endpoint_path=effective_public_endpoint_path,
            model_name=model_name,
            route_context=route_context,
            ...
            raise RouteExhaustedRetrySignal(
                ...
                route_retry_failed_candidate_keys=failed_candidate_keys,
            )
        ...
        try:
            ...
            if model_mapping_unavailable:
                ...
                route_decision = await RoutingService.select_candidates(
                    RouteRequest(
                        ...
                        endpoint_path=endpoint_path,
                        public_endpoint_path=effective_public_endpoint_path,
                        ...
                        excluded_candidate_keys=set(failed_candidate_keys),
                    )
                )
                candidates = route_decision.candidates
        except ProviderCapacityUnavailableError as exc:
            raise ProxyService._capacity_service_unavailable_http_exception(exc) from exc
        route_diagnostics_for_trace = (
            None
            if model_mapping_unavailable
            else route_decision.diagnostics.to_dict() if route_decision is not None else None
        )
        if candidates and failed_candidate_keys:
            candidates = RoutingService.filter_failed_candidates(candidates, failed_candidate_keys)
        if candidates:
            ...
            ProxyService._append_route_decision_trace_event(
                ...
                selected_reason=first_candidate.selection_reason,
                top_candidates=route_decision.top_candidates_summary() if route_decision is not None else RoutingService.route_candidate_trace_summary(candidates),
                failed_candidate_keys=failed_candidate_keys,
                sticky_hit=bool(session_sticky_key),
                diagnostics=route_diagnostics_for_trace,
                stage_traces=route_decision.stage_traces_payload() if route_decision is not None else None,
            )
            await ProxyService._upsert_live_request_log_async(
                db,
                base_log_kwargs=base_log_kwargs,
                trace=trace,
                request_status=LogService.REQUEST_STATUS_REQUESTING,
                message="已选定候选提供商，准备发起上游请求",
                attempt_count=attempt_count,
                provider=first_candidate.provider,
                provider_model=first_candidate.provider_model,
            )
        if not candidates:
            route_diagnostics = (
                ProxyService._model_mapping_unavailable_diagnostics(mapping_resolution)
                if model_mapping_unavailable
                else route_diagnostics_for_trace
                or await ProxyService._route_diagnostics_for_trace(
                    model_name=model_name,
                    forced_provider_id=forced_provider_id,
                    route_context=route_context,
                    require_vision=has_image_input,
            ...
            route_error_detail = ProxyService._build_route_unavailable_error_detail(
                ...
                requested_endpoint_override=effective_log_request_path,
            )
            ...
            if mapped_retry is not None:
                return mapped_retry
            retryable_route_exhausted = RoutingService.should_retry_route_diagnostics(route_diagnostics)
            retry_wait_plan = (
                RoutingService.route_retry_wait_plan(
                    setting,
            ...
            if sleep_seconds > 0:
                RoutingService.remember_route_outage(
                    ...
                    diagnostics=route_diagnostics,
                    error_code_resolver=ProxyService._error_code_from_detail,
                )
            ...
            if retryable_route_exhausted and not RoutingService.route_retry_infinite_enabled(setting, route_context) and RoutingService.route_retry_max_wait_seconds(setting) > 0:
                await ProxyService._raise_final_error_async(
                    ...
                    trace=[
                        *trace,
                        RoutingService.route_candidates_exhausted_trace_item(
                            route_diagnostics,
                            failed_candidate_keys=failed_candidate_keys,
                        ),
                    ],
        ...
        for candidate in candidates:
            ...
            if balance_rejection is not None:
                ...
                RoutingService.mark_candidate_key_failed(failed_candidate_keys, candidate)
                await ProxyService._upsert_live_request_log_async(
                    db,
                    base_log_kwargs=base_log_kwargs,
                    trace=trace,
                    request_status=LogService.REQUEST_STATUS_REQUESTING,
                    message="候选提供商余额预检查失败，切换下一个候选",
                    attempt_count=attempt_count,
                    provider=provider,
                    provider_model=provider_model,
                )
            ...
            for retry_index in range(retries):
                ...
                try:
                    async with ProviderCapacityService.async_lease(provider, is_stream=False, provider_model=provider_model):
                    ...
                    response_body_json = ProxyService._serialize_response_payload_for_logging(client_response, setting=setting)
                    response_text = (
                    ...
                    detection_response_text = response_text or ProxyService._extract_response_display_text(
                        client_response,
                        limit_bytes=min(4096, max(1024, int(getattr(setting, "max_logged_body_bytes", 4096) or 4096))),
                    )
                    ...
                    guard_blocked = ProxyService._content_guard_should_block_for_response(guard_result, setting=setting)
                    if guard_blocked:
                        if ContentRuntimeGuardService.should_emit_safe_error(guard_result, setting=setting):
                            guard_result.final_strategy = guard_result.final_strategy or "safe_error"
                            if ProxyService._should_append_content_guard_typed_event(guard_result, setting=setting, blocked=True):
                                ProxyService._append_typed_request_event(
                                    trace,
                                    "request_content_guard",
                                    ProxyService._content_guard_typed_payload(
                                        guard_result,
                            ...
                            safe_response = ProxyService._build_content_guard_safe_response(
                                request_payload=payload,
                                endpoint_path=endpoint_path,
                                model_name=requested_model_name,
                                trace_id=trace_id,
                            )
                            response_body_json = ProxyService._serialize_response_payload_for_logging(safe_response, setting=setting)
                            response_text = (
                                ProxyService._extract_response_display_text(safe_response, limit_bytes=setting.max_logged_body_bytes)
                                if setting.enable_payload_logging
                                else None
                            )
                            trace.append(
                            ...
                            if not suppress_success_log:
                                await ProxyService._run_db_write(
                                    ProxyService._upsert_success_log_with_provider_status,
                                    log_type=log_type,
                                    trace_id=trace_id,
                                    provider_id=provider.id,
                                    provider_name=provider.name,
                                    ...
                                    status_code=200,
                                    latency_ms=latency_ms,
                                    duration_ms=ProxyService._elapsed_ms_since(
                                        request_started_at,
                                    ...
                                    completion_tokens=0,
                                    total_tokens=usage_info["prompt_tokens"],
                                    ...
                                    message="content guard safe error fallback",
                                    retryable=False,
                                    **{
                                        **guard_result.to_log_kwargs(),
                                        "content_guard_final_strategy": guard_result.final_strategy or "safe_error",
                                    },
                                    content_guard_retry_provider_count=ProxyService._content_guard_retry_provider_count(trace),
                                    **ProxyService._build_api_client_log_kwargs(api_client_auth, auth_result="authenticated"),
                                    **ProxyService._build_provider_log_kwargs(provider_model),
                                    trace=trace,
                                    token_request_payload=payload,
                                    token_response_payload=safe_response,
                                    token_response_text=response_text,
                                    schedule_token_fill=False,
                                )
                            ProxyService._release_owner_balance_reservation(api_client_auth, trace_id=trace_id)
                            return safe_response, provider, trace, latency_ms
                        trace.append(
                            ProxyService._build_trace_item(
                                provider,
                                provider_model,
                        ...
                        if ProxyService._content_guard_should_switch_provider(setting):
                            guard_result.final_strategy = guard_result.final_strategy or "switch_provider"
                            if ProxyService._should_append_content_guard_typed_event(guard_result, setting=setting, blocked=True):
                                ProxyService._append_typed_request_event(
                                    trace,
                                    "request_content_guard",
                        ...
                        guard_result.final_strategy = guard_result.final_strategy or "block"
                        if ProxyService._should_append_content_guard_typed_event(guard_result, setting=setting, blocked=True):
                            ProxyService._append_typed_request_event(
                                trace,
                                "request_content_guard",
                    ...
                    if ProxyService._should_append_content_guard_typed_event(guard_result, setting=setting, blocked=False):
                        ProxyService._append_typed_request_event(
                            trace,
                            "request_content_guard",
                            ProxyService._content_guard_typed_payload(
                    ...
                    fixed_success_detection = FixedSuccessResponseService.inspect_and_record(
                        provider_id=provider.id,
                        provider_model_id=provider_model.id,
                        request_payload=request_payload_for_log,
                        response_text=detection_response_text,
                    )
                    if fixed_success_detection.get("detected"):
                        confirmation = await ProxyService._confirm_fixed_success_response_detection(
                            provider,
                            provider_model,
                            fixed_success_detection,
                            trace=trace,
                        )
                        if confirmation.get("should_mark_unhealthy"):
                            fixed_success_detection["will_switch_provider"] = True
                            ProxyService._append_typed_request_event(
                                trace,
                                "fixed_success_response_detected",
                                fixed_success_detection,
                                result="failed",
                                severity="warning",
                                module="routing",
                            ...
                            trace.append(
                                ProxyService._build_trace_item(
                                    ...
                                    error=ProviderHealthStateService.FIXED_SUCCESS_RESPONSE_ERROR_CODE,
                                    extra={
                                        "similarity": fixed_success_detection.get("similarity"),
                                        "threshold": fixed_success_detection.get("threshold"),
                                        "request_fingerprint": fixed_success_detection.get("request_fingerprint"),
                                        "previous_request_fingerprint": fixed_success_detection.get("previous_request_fingerprint"),
                                        "fixed_answer_confirmation": confirmation,
                                    },
                                )
                            )
                            await ProxyService._mark_fixed_success_response_failure_async(
                                provider,
                                provider_model,
                                latency_ms,
                                fixed_success_detection,
                            ...
                            raise FixedSuccessResponseDetectedError(detection=fixed_success_detection)
                        fixed_success_detection["will_switch_provider"] = False
                        ProxyService._append_typed_request_event(
                            trace,
                            "fixed_success_response_detected",
                            fixed_success_detection,
                            ...
                            **{
                                **guard_result.to_log_kwargs(),
                                "content_guard_final_strategy": (
                                    ...
                                    if ProxyService._content_guard_retry_provider_count(trace) > 0
                                    else guard_result.final_strategy
                                ),
                            },
                            content_guard_retry_provider_count=ProxyService._content_guard_retry_provider_count(trace),
                            **ProxyService._build_api_client_log_kwargs(api_client_auth, auth_result="authenticated"),
                    ...
                    if suppress_success_log or not setting.enable_token_logging:
                        ProxyService._release_owner_balance_reservation(api_client_auth, trace_id=trace_id)
                    ...
                    await ProxyService._upsert_live_request_log_async(
                        ...
                        trace=trace,
                        request_status=LogService.REQUEST_STATUS_REQUESTING,
                        message="内容防护阻断，切换候选提供商",
                        attempt_count=attempt_count,
                        provider=provider,
                        provider_model=provider_model,
                    )
                    break
                except FixedSuccessResponseDetectedError as exc:
                    last_upstream_error = {
                        "status_code": exc.status_code,
                        "detail": exc.detail,
                    }
                    ...
                    last_upstream_error = {
                        ...
                        "retry_after_seconds": ProxyService._retry_after_seconds_from_headers(exc.response.headers),
                    }
                    ...
                    if not ProxyService._should_retry_same_provider_status(
                        ...
                        detail=last_upstream_error,
                    ):
                        break
                    await ProxyService._sleep_before_same_provider_retry(
                        trace,
                        provider=provider,
                        provider_model=provider_model,
                    ...
                    last_upstream_error = {
                        "status_code": exc.status_code,
                        "detail": exc.detail,
                        "retry_after_seconds": ProxyService._retry_after_seconds_from_detail(exc.detail),
                    }
                    await ProxyService._mark_failure_async(
                        provider,
                        provider_model,
        ...
        ProxyService._append_failed_candidate_exclusion_trace_event(
            trace,
            route_round=route_retry_round + 1,
            failed_candidate_keys=failed_candidate_keys,
        )
        ...
        if ProxyService._should_retry_route_upstream_error(last_upstream_error):
            retry_wait_plan = RoutingService.route_retry_wait_plan(
                setting,
                started_at=route_retry_started_at,
                retry_round=route_retry_round,
                route_context=route_context,
            ...
            if not RoutingService.route_retry_infinite_enabled(setting, route_context) and RoutingService.route_retry_max_wait_seconds(setting) > 0:
    ...
    async def forward_stream_request(db: Session | None = None, **kwargs) -> tuple[AsyncIterator[bytes], Provider, list[dict], int]:
        if db is None:
            with SessionLocal() as scoped_db:
                return await ProxyService.forward_stream_request(scoped_db, **kwargs)
        route_retry_started_at = kwargs.pop("route_retry_started_at", None)
        route_retry_round = int(kwargs.pop("route_retry_round", 0) or 0)
        route_retry_trace = kwargs.pop("route_retry_trace", None)
        route_retry_attempt_count = int(kwargs.pop("route_retry_attempt_count", 0) or 0)
        ...
        *,
        ...
        for candidate in candidates:
            ...
            for retry_index in range(retries):
                ...
                try:
                    capacity_lease = ProviderCapacityService.async_lease(provider, is_stream=True, provider_model=provider_model)
                    ...
                    if stream_guard_enabled and ProxyService._content_guard_should_switch_provider(setting):
                        stream_guard_buffering = True
                    guard_buffer_limit = min(
                        CONTENT_GUARD_STREAM_BUFFER_MAX_BYTES_LIMIT,
                        max(1024, int(getattr(setting, "content_guard_stream_buffer_max_bytes", 16384) or 16384)),
                    )
                    guard_max_delay_ms = min(500, max(0, int(getattr(setting, "content_guard_max_detection_delay_ms", 300) or 0)))
                    if stream_guard_mode == "full_buffer":
                        guard_max_delay_ms = min(500, max(300, guard_max_delay_ms))
                    ...
                    if stream_guard_buffering and prefetched_chunk_iterator is not None:
                        (
                            ...
                            stop_at_first_event=False,
                            route_context=route_context,
                            full_buffer_mode=stream_guard_mode == "full_buffer",
                            timeout_policy=ProxyService._build_stream_timeout_policy(provider=provider, setting=setting)
                            if stream_guard_mode == "full_buffer"
                            else None,
                        )
                        if (
                            prefetch_pending_read_task is not None
                            and not prefetched_stream_guard_bytes
                            and not prefetched_stream_ended
                            and ProxyService._content_guard_should_switch_provider(setting)
                        ):
                            (
                                prefetched_stream_guard_bytes,
                                prefetch_guard_result,
                        ...
                        if ProxyService._content_guard_should_block_for_response(prefetch_guard_result, setting=setting):
                            ...
                            await ProxyService._upsert_live_request_log_async(
                                ...
                                trace=trace,
                                request_status=LogService.REQUEST_STATUS_REQUESTING,
                                message="流式首段命中内容防护，切换候选提供商",
                                attempt_count=attempt_count,
                                provider=provider,
                                provider_model=provider_model,
                            )
                            break
                        prefetch_response_text = ProxyService._extract_stream_prefetch_display_text(
                            prefetched_stream_guard_bytes,
                        ...
                        fixed_success_detection = FixedSuccessResponseService.inspect_and_record(
                            ...
                            provider_model_id=provider_model.id,
                            request_payload=request_payload_for_log,
                            response_text=prefetch_response_text,
                        )
                        if fixed_success_detection.get("detected"):
                            duration_ms = int((time.perf_counter() - started) * 1000)
                            confirmation = await ProxyService._confirm_fixed_success_response_detection(
                            ...
                            if confirmation.get("should_mark_unhealthy"):
                                ...
                                last_upstream_error = {
                                    "status_code": status.HTTP_502_BAD_GATEWAY,
                                    "detail": {
                                        "message": "上游流式首段返回了高度相似的固定失败文本，已按伪成功响应故障切换候选",
                                        "code": ProviderHealthStateService.FIXED_SUCCESS_RESPONSE_ERROR_CODE,
                                        "detection": fixed_success_detection,
                                    },
                                }
                                RoutingService.mark_candidate_key_failed(failed_candidate_keys, candidate)
                                await ProxyService._upsert_live_request_log_async(
                                ...
                                break
                            fixed_success_detection["will_switch_provider"] = False
                            fixed_success_detection["downstream_already_started"] = False
                            ProxyService._append_typed_request_event(
                                trace,
                                "fixed_success_response_detected",
                                fixed_success_detection,
                    ...
                    async def stream_generator() -> AsyncIterator[bytes]:
                        ...
                        stream_guard_text_window = bytearray()
                        guard_result = prefetch_guard_result
                        ...
                        try:
                                ...
                                if upstream_chunk:
                                    if stream_guard_enabled:
                                        current_guard_result = ProxyService._inspect_stream_guard_chunk(
                                            ...
                                            provider=provider,
                                            route_context=route_context,
                                            text_window=stream_guard_text_window,
                                        )
                                    ...
                                    if stream_guard_enabled:
                                        if ProxyService._content_guard_should_block_for_response(current_guard_result, setting=setting):
                                            violation_latency_ms = int((time.perf_counter() - started) * 1000)
                                            trace.append(
                                                ProxyService._build_trace_item(
                                                    provider,
                                                    provider_model,
                                                    "content_integrity_violation",
                                                    violation_latency_ms,
                                                    status_code=ContentRuntimeGuardService.CONTENT_GUARD_ERROR_STATUS_CODE,
                                                    error=current_guard_result.reason,
                                                )
                                            )
                                            await ProxyService._run_db_write(
                                            ...
                                            return
                                    if first_chunk_latency_ms is None:
                                        first_chunk_seen_at = time.perf_counter()
                                        first_chunk_latency_ms = (
                                            prefetch_first_chunk_latency_ms
                                            if prefetch_first_chunk_latency_ms is not None
                                            else int((first_chunk_seen_at - started) * 1000)
                                        )
                                        stream_ttfb_ms = ProxyService._stream_ttfb_ms(
                                            request_started_at=request_started_at,
                            ...
                            end_to_end_duration_ms = (
                                ProxyService._elapsed_ms_since(request_started_at, fallback_started_at=started)
                            ...
                            base_log_kwargs = {
                                ...
                                "ttfb_ms": stream_ttfb_ms if stream_ttfb_ms is not None else first_chunk_latency_ms,
                                "duration_ms": end_to_end_duration_ms,
                                "upstream_duration_ms": total_duration_ms,
                                "reasoning_level": reasoning_level,
                                "model_reasoning_effort": model_reasoning_effort,
                                "attempt_count": attempt_count,
                                "prompt_tokens": usage_info["prompt_tokens"],
                            ...
                            stream_guard_blocked = error_code == "content_integrity_violation"
                            stream_billable = (
                                success
                                and not interrupted
                            ) or (
                                ...
                                and (
                                    int(usage_info.get("total_tokens") or 0) > 0
                                    or bool(str(response_text_for_log or "").strip())
                                )
                            )
                            if ProxyService._should_append_content_guard_typed_event(guard_result, setting=setting, blocked=stream_guard_blocked):
                                ProxyService._append_typed_request_event(
                                    typed_terminal_events,
                                    "request_content_guard",
                                    ProxyService._content_guard_typed_payload(
                                    ...
                                    result="blocked" if stream_guard_blocked else "success",
                                    severity="danger" if guard_result.risk_level == "high" else "info",
                                    module="content_guard",
                                )
                            ProxyService._append_typed_request_event(
                                ...
                                {
                                    "billing_event_id": None,
                                    "billing_stage": "queued" if setting.enable_token_logging and stream_billable else "skipped",
                                    "token_source": "upstream_usage" if usage_info.get("total_tokens") is not None else "missing",
                                    "prompt_tokens": usage_info["prompt_tokens"],
                                    "completion_tokens": usage_info["completion_tokens"],
                                    "cache_read_tokens": usage_info["cache_read_tokens"],
                            ...
                            if success and not interrupted:
                                post_stream_fixed_success_detection = FixedSuccessResponseService.inspect_and_record(
                                    provider_id=provider.id,
                                    provider_model_id=provider_model.id,
                                    request_payload=request_payload_for_log,
                                    response_text=response_text_for_log,
                                )
                                if post_stream_fixed_success_detection.get("detected"):
                                    confirmation = await ProxyService._confirm_fixed_success_response_detection(
                                        provider,
                                        provider_model,
                                        post_stream_fixed_success_detection,
                                        trace=typed_terminal_events,
                                    )
                                    post_stream_fixed_success_detection["will_switch_provider"] = False
                                    post_stream_fixed_success_detection["downstream_already_started"] = True
                                    if confirmation.get("should_mark_unhealthy"):
                                        ProxyService._append_typed_request_event(
                                            typed_terminal_events,
                                            "fixed_success_response_detected",
                                        ...
                                        fixed_success_trace = trace + typed_terminal_events + [
                                            ProxyService._build_trace_item(
                                                provider,
                                                provider_model,
                                                "fixed_success_response_detected",
                                                ...
                                                error=ProviderHealthStateService.FIXED_SUCCESS_RESPONSE_ERROR_CODE,
                                                extra={
                                                    "similarity": post_stream_fixed_success_detection.get("similarity"),
                                                    "threshold": post_stream_fixed_success_detection.get("threshold"),
                                                    "request_fingerprint": post_stream_fixed_success_detection.get("request_fingerprint"),
                                                    "previous_request_fingerprint": post_stream_fixed_success_detection.get("previous_request_fingerprint"),
                                                    "downstream_already_started": True,
                                                    "fixed_answer_confirmation": confirmation,
                                                },
                                            )
                                        ]
                                        await ProxyService._run_db_write(
                                            ...
                                            **{
                                                **base_log_kwargs,
                                                ...
                                                "billable": False,
                                                "status_code": status.HTTP_502_BAD_GATEWAY,
                                                "message": "stream fixed success response detected after downstream output",
                                                "error_type": "server_error",
                                                "error_code": ProviderHealthStateService.FIXED_SUCCESS_RESPONSE_ERROR_CODE,
                                                "retryable": True,
                                                "trace": fixed_success_trace,
                                                "schedule_token_fill": False,
                                            },
                                        )
                                        ProxyService._release_owner_balance_reservation(api_client_auth, trace_id=trace_id)
                                ...
                                terminal_status_code = (
                                    ...
                                    if client_cancelled
                                        ...
                                        ContentRuntimeGuardService.CONTENT_GUARD_ERROR_STATUS_CODE
                                        if error_code == "content_integrity_violation"
                                        else (502 if interrupted or not success else 200)
                                    )
                                )
                                ...
                                await ProxyService._run_db_write(
                                    ...
                                    **{
                                        **base_log_kwargs,
                                        "success": False,
                                        "billable": stream_billable,
                                        "billable_reason": "blocked_after_output" if error_code == "content_integrity_violation" else None,
                                        "status_code": terminal_status_code,
                                        "message": error_message or f"stream {log_type} failed",
                                        "error_type": (
                                            "content_policy_error"
                                            if error_code == "content_integrity_violation"
                                            else ("server_error" if not client_cancelled else "client_error")
                                        ),
                                        "error_code": error_code,
                                        "retryable": False if error_code == "content_integrity_violation" else not client_cancelled,
                                        "trace": interrupted_trace,
                                        **(
                                            {
                                                **guard_result.to_log_kwargs(),
                                                "content_guard_final_strategy": (
                                                    guard_result.final_strategy
                                                    or (
                                                        "stream_interrupted"
                                                        if error_code == "content_integrity_violation"
                                                        else None
                            ...
                            if not (setting.enable_token_logging and stream_billable):
                                ProxyService._release_owner_balance_reservation(api_client_auth, trace_id=trace_id)
    ...
    def _get_thread_local_requests_session() -> requests.Session:
        ...
        pool_size = max(
            1,
            min(
                int(getattr(settings, "upstream_max_keepalive_connections", 50) or 50),
                int(getattr(settings, "upstream_max_connections", 200) or 200),
            ),
        )
        ...
        adapter = HTTPAdapter(
            ...
            max_retries=0,
            pool_block=bool(getattr(settings, "upstream_requests_pool_block", True)),
        )
    ...
    @staticmethod
    def _get_requests_connection_semaphore() -> threading.BoundedSemaphore:
        settings = get_settings()
        max_connections = max(1, int(getattr(settings, "upstream_max_connections", 200) or 200))
        fingerprint = (max_connections,)
        if (
            ProxyService._requests_connection_semaphore is not None
            and ProxyService._requests_connection_semaphore_fingerprint == fingerprint
        ):
            return ProxyService._requests_connection_semaphore
        with ProxyService._requests_connection_semaphore_guard:
            if (
                ProxyService._requests_connection_semaphore is None
                or ProxyService._requests_connection_semaphore_fingerprint != fingerprint
            ):
                ProxyService._requests_connection_semaphore = threading.BoundedSemaphore(max_connections)
                ProxyService._requests_connection_semaphore_fingerprint = fingerprint
        return ProxyService._requests_connection_semaphore

    @staticmethod
    @contextmanager
    def _requests_connection_slot():
        settings = get_settings()
        semaphore = ProxyService._get_requests_connection_semaphore()
        acquired = semaphore.acquire(
            timeout=max(0.001, float(getattr(settings, "upstream_pool_timeout_s", 10.0) or 10.0)),
        )
        if not acquired:
            raise httpx.PoolTimeout("upstream requests connection pool timeout")
        try:
            yield
        finally:
            semaphore.release()
    ...
    async def _raise_stream_response_for_status(response: UpstreamStreamResponse) -> None:
        ...
        if response.status_code < 400:
            return
        body = await ProxyService._read_limited_upstream_error_body(response)
        error_response = response.to_httpx_response(content=body)
        error_response.raise_for_status()

    @staticmethod
    async def _read_limited_upstream_error_body(response: UpstreamStreamResponse) -> bytes:
        if isinstance(response, httpx.Response):
            return await ProxyService._read_limited_httpx_response(
                response,
                max_response_bytes=ProxyService.UPSTREAM_ERROR_BODY_LIMIT_BYTES,
            )
        body = bytearray()
        async for chunk in response.aiter_bytes():
            if not chunk:
                continue
            if len(body) + len(chunk) > ProxyService.UPSTREAM_ERROR_BODY_LIMIT_BYTES:
                body.extend(chunk[: max(0, ProxyService.UPSTREAM_ERROR_BODY_LIMIT_BYTES - len(body))])
                break
            body.extend(chunk)
        return bytes(body)
    ...
    def _should_retry_same_provider_status(status_code: int, detail: Any | None = None) -> bool:
        if status_code == status.HTTP_429_TOO_MANY_REQUESTS and ProxyService._retry_after_seconds_from_detail(detail) is not None:
            return True
        classified = ProxyService._classify_retry_policy(status_code, detail)
    ...
    def _same_provider_retry_budget(provider: Provider, setting: Any) -> int:
        configured_budget = min(int(provider.max_retries or 1), int(setting.global_max_retries or 1))
        return max(1, min(configured_budget, 2))

    @staticmethod
    def _same_provider_retry_wait_plan(
        *,
        retry_index: int,
        upstream_error: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        retry_after_seconds = ProxyService._retry_after_seconds_from_upstream_error(upstream_error)
        jitter_ratio: float | None = None
        if retry_after_seconds is not None:
            retry_after_jitter_seconds = random.uniform(0.0, min(0.3, max(0.05, float(retry_after_seconds) * 0.1)))
            wait_seconds = min(float(retry_after_seconds) + retry_after_jitter_seconds, 10.0)
            wait_source = "retry_after"
            jitter_ratio = None
        else:
            retry_after_jitter_seconds = None
            base_wait_seconds = min(2.0, 0.25 * (2 ** max(0, retry_index)))
            jitter_ratio = random.uniform(-0.2, 0.2)
            wait_seconds = max(0.05, base_wait_seconds * (1.0 + jitter_ratio))
            wait_source = "backoff_jitter"
        return {
    ...
    async def _sleep_before_same_provider_retry(
        ...
        *,
        ...
        retries: int,
        upstream_error: dict[str, Any] | None = None,
        route_round: int | None = None,
        endpoint_path: str | None = None,
        db: Session | None = None,
        base_log_kwargs: dict[str, Any] | None = None,
        ...
        sleep_seconds = float(wait_plan.get("sleep_seconds") or 0.0)
        if sleep_seconds <= 0:
            return
        trace.append(
            ProxyService._build_trace_item(
                provider,
                provider_model,
                "same_provider_retry_wait",
                0,
                extra={
                    "route_round": route_round,
                    "retry_index": retry_index,
                    "endpoint_path": endpoint_path,
                    "retry_wait_seconds": round(sleep_seconds, 3),
                    "retry_wait_plan": wait_plan,
                },
            )
        )
        if base_log_kwargs is not None:
            await ProxyService._upsert_live_request_log_async(
                db,
                base_log_kwargs=base_log_kwargs,
                trace=trace,
                request_status=LogService.REQUEST_STATUS_REQUESTING,
                message="上游失败，等待同提供商重试",
    ...
    @staticmethod
    def _capacity_service_unavailable_detail(exc: ProviderCapacityUnavailableError) -> dict[str, Any]:
        return {
            "message": (
                "Redis 容量服务不可用，项目对外代理服务已临时不可用。"
                "为避免共享并发和限流失效，当前请求已拒绝；请先恢复 Redis 后再继续服务。"
            ),
            "code": "provider_capacity_service_unavailable",
            "reason": str(exc),
    ...
    @staticmethod
    def _capacity_service_unavailable_http_exception(exc: ProviderCapacityUnavailableError) -> HTTPException:
        return HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=ProxyService._capacity_service_unavailable_detail(exc),
        )
    ...
    def _retry_after_seconds_from_headers(headers: Any) -> float | None:
        ...
        for key in ("Retry-After", "retry-after"):
            ...
            if raw_value:
                break
        return ProxyService._parse_retry_after_seconds(raw_value)

    @staticmethod
    def _retry_after_seconds_from_detail(detail: Any) -> float | None:
        if not isinstance(detail, dict):
            return None
        for key in ("retry_after_seconds", "retry_after_sec", "retry_after_ms", "retry_after", "Retry-After"):
            if key not in detail:
                continue
            parsed = RoutingService.parse_retry_after_seconds(detail.get(key), milliseconds=key == "retry_after_ms")
            if parsed is not None:
                return parsed
        error = detail.get("error")
        if isinstance(error, dict):
            return RoutingService.retry_after_seconds_from_detail(error)
        return None

    @staticmethod
    def _retry_after_seconds_from_upstream_error(upstream_error: dict[str, Any] | None) -> float | None:
        if not isinstance(upstream_error, dict):
            return None
        parsed = RoutingService.parse_retry_after_seconds(upstream_error.get("retry_after_seconds"))
        if parsed is not None:
            return parsed
        return RoutingService.retry_after_seconds_from_detail(upstream_error.get("detail"))

    @staticmethod
    def _parse_retry_after_seconds(value: Any, *, milliseconds: bool = False) -> float | None:
        return RoutingService.parse_retry_after_seconds(value, milliseconds=milliseconds)
    ...
    def _append_route_exhausted_retry_trace(
        ...
        *,
        ...
        infinite_retry_enabled = RoutingService.route_retry_infinite_enabled(setting, route_context)
        wait_plan = retry_wait_plan or {}
        item: dict[str, Any] = {
            "result": "route_exhausted_wait_retry",
            "reason": reason,
            "retry_round": retry_round + 1,
            "sleep_seconds": round(sleep_seconds, 3),
            "elapsed_seconds": round(RoutingService.route_retry_elapsed_seconds(started_at=started_at), 3),
            "max_wait_seconds": RoutingService.route_retry_max_wait_seconds(setting),
            "infinite_retry_enabled": infinite_retry_enabled,
            "retry_mode": "infinite" if infinite_retry_enabled else "normal",
            "wait_source": wait_plan.get("wait_source"),
            "base_wait_seconds": wait_plan.get("base_wait_seconds"),
            "jitter_ratio": wait_plan.get("jitter_ratio"),
            "retry_after_seconds": wait_plan.get("retry_after_seconds"),
            "remaining_wait_seconds": wait_plan.get("remaining_wait_seconds"),
            "limited_by_wait_window": wait_plan.get("limited_by_wait_window"),
        }
        ...
        if len(trace) >= ProxyService.ROUTE_RETRY_TRACE_LIMIT:
            del trace[: len(trace) - ProxyService.ROUTE_RETRY_TRACE_LIMIT + 1]
        trace.append(item)
        ProxyService._append_route_decision_trace_event(
            trace,
            route_round=retry_round + 1,
            candidate_count=0,
            diagnostics=diagnostics,
            retry_wait_ms=int(round(sleep_seconds * 1000)),
            retry_wait_plan=wait_plan,
            selection_guard="route_exhausted_wait_retry",
        )
    ...
    def _build_route_exhausted_retry_upstream_error(
        ...
        *,
        ...
        return RoutingService.build_route_exhausted_retry_upstream_error(
            setting,
    ...
    def _prepare_upstream_request(
        ...
        *,
        ...
        public_model_name = internal_payload.get("model")
        ...
        if response_model_override is None and isinstance(public_model_name, str) and public_model_name.strip():
            response_model_override = public_model_name.strip()
        ...
        normalized_payload = ProxyService._normalize_provider_request_payload(
            provider,
            endpoint_path=upstream_endpoint_path,
    ...
    @staticmethod
    def _normalize_provider_request_payload(
        provider: Provider,
        *,
        endpoint_path: str,
        payload: dict[str, Any],
        ...
        if provider_model is not None and "model" in normalized_payload:
            upstream_model_name = ProviderService.provider_model_upstream_model_name(
                provider_model,
                fallback_model=str(normalized_payload.get("model") or ""),
            )
            if upstream_model_name and upstream_model_name != normalized_payload.get("model"):
                normalized_payload = dict(normalized_payload)
                normalized_payload["model"] = upstream_model_name
        if endpoint_path == "/chat/completions" and ProxyService._provider_requires_max_completion_tokens(provider, normalized_payload):
    ...
    def _fetch_remote_image_as_data_url(url: str) -> str:
        max_image_bytes = AssetService.MAX_IMAGE_BYTES
        timeout_seconds = 30
        session = ProxyService._get_thread_local_requests_session()
        try:
            with session.get(url, timeout=timeout_seconds, stream=True) as response:
                ...
                try:
                    ...
                    response_bytes = None
                if response_bytes is not None and response_bytes > max_image_bytes:
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail={
                ...
                if content_type and content_type.lower() not in AssetService.IMAGE_CONTENT_TYPES:
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail={
                            "message": "远程 URL 不是受支持的图片类型，请改为传入 PNG/JPEG/WEBP/GIF 图片或可用文件 ID",
                            "code": "image_url_unsupported_content_type",
                            "image_url": url,
    ...
    def _append_route_decision_trace_event(
        ...
        *,
        ...
        if selection_guard:
            payload["selection_guard"] = selection_guard
        ProxyService._append_typed_request_event(
            trace,
    ...
    @staticmethod
    def _append_failed_candidate_exclusion_trace_event(
        trace: list[dict],
        *,
        route_round: int,
        failed_candidate_keys: set[tuple[int, int]],
    ) -> None:
        ...
        ProxyService._append_typed_request_event(
            ...
            {
                "route_round": route_round,
                "failed_candidate_keys_json": dumps_json(RoutingService.serialize_failed_candidate_keys(failed_candidate_keys)),
                "failed_candidate_count": len(failed_candidate_keys),
                "selection_guard": "exclude_failed_candidates_in_current_round",
            },
            result="failed",
            severity="warning",
        )
    ...
    @staticmethod
    async def _route_diagnostics_for_trace(
        *,
        model_name: str | None,
        ...
        try:
            diagnostics = await RoutingService.diagnose(
                RouteRequest(
            ...
            logger.debug("Failed to collect route diagnostics for trace: %s", exc)
            return None
    ...
    def _build_route_unavailable_error_detail(
        ...
        *,
        ...
        requested_endpoint_override: str | None = None,
    ) -> dict[str, Any]:
        requested_endpoint = requested_endpoint_override or ProxyService._external_v1_endpoint(endpoint_path)
        ...
        protocol_detail = ProxyService._protocol_unavailable_detail(route_diagnostics, requested_endpoint=requested_endpoint)
        if protocol_detail is not None:
            detail.update(protocol_detail)
            detail["message"] = protocol_detail["message"]
            detail["code"] = protocol_detail["code"]
        return detail
    ...
    @staticmethod
    def _protocol_unavailable_detail(
        route_diagnostics: dict[str, Any],
        *,
        requested_endpoint: str,
    ) -> dict[str, Any] | None:
        reason_counts = route_diagnostics.get("reason_counts") or {}
        if int(reason_counts.get("protocol_mismatch") or 0) <= 0:
            return None
        samples = [
            item
            for item in (route_diagnostics.get("samples") or [])
            if isinstance(item, dict) and item.get("protocol_mismatch")
        ]
        required_protocol = str(
            route_diagnostics.get("required_upstream_protocol_type")
            or (samples[0].get("required_protocol") if samples else "")
            or ""
        )
        actual_protocols = sorted(
            {
                str(item.get("actual_protocol"))
                for item in samples
                if item.get("actual_protocol")
            }
        )
        return {
            "message": (
                f"请求入口 {requested_endpoint} 需要上游原生协议 {required_protocol or 'unknown'}，"
                "但当前候选提供商或模型的实际协议不匹配。"
            ),
            "code": "upstream_protocol_mismatch",
            "requested_endpoint": requested_endpoint,
            "required_protocol": required_protocol or None,
            "actual_protocol": actual_protocols[0] if len(actual_protocols) == 1 else None,
            "actual_protocols": actual_protocols,
            "primary_reason": {
                "code": "protocol_mismatch",
                "label": RoutingService.diagnostic_reason_label("protocol_mismatch"),
            },
            "reason_counts": reason_counts,
            "reason_details": ProxyService._route_reason_details(route_diagnostics),
            "diagnostic_samples": samples or route_diagnostics.get("samples") or [],
        }
        ...
        *,
        ...
        return {
            ...
            "primary_reason": {
                ...
                "label": RoutingService.diagnostic_reason_label(primary_reason_code) if primary_reason_code else None,
            },
            "reason_counts": route_diagnostics.get("reason_counts") or {},
        ...
        return [
            {
                ...
                "label": RoutingService.diagnostic_reason_label(str(reason_code)),
    ...
    @staticmethod
    def _build_content_guard_safe_response(
        *,
        request_payload: dict[str, Any],
        endpoint_path: str,
        model_name: str | None,
        ...
        message = "响应未通过内容完整性防护，系统已返回安全兜底内容。"
        resolved_model = model_name or str(request_payload.get("model") or "content-guard-safe")
        ...
        if endpoint_path == "/responses":
            return {
                ...
                "output_text": message,
                "finish_reason": "content_filter",
                "metadata": {"trace_id": trace_id, "content_guard_action": "safe_error"},
                "usage": {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0},
            }
        if endpoint_path == "/completions":
            return {
                ...
                "created": created_at,
                "model": resolved_model,
                "choices": [{"text": message, "index": 0, "finish_reason": "content_filter"}],
    ...
    @staticmethod
    def _format_stream_error_event(
        *,
        message: str,
        code: str,
        trace_id: str | None,
        ...
        status_code = (
            ContentRuntimeGuardService.CONTENT_GUARD_ERROR_STATUS_CODE
            if code == "content_integrity_violation"
    ...
    async def _extract_response_error(response: httpx.Response) -> str:
        try:
            body = await ProxyService._read_limited_httpx_response(
                response,
                max_response_bytes=ProxyService.UPSTREAM_ERROR_BODY_LIMIT_BYTES,
            )
        except Exception:
    ...
    def _raise_final_error(
        ...
        *,
        ...
        duration_ms = ProxyService._elapsed_ms_since(request_started_at)
        upstream_duration_ms = ProxyService._latest_upstream_duration_ms_from_trace(trace)
        ...
        if content_guard_exhausted:
            ...
            if isinstance(detail, dict) and isinstance(detail.get("error"), dict):
                error_detail = detail["error"].get("detail")
                guard = error_detail.get("content_guard") if isinstance(error_detail, dict) else {}
            if isinstance(guard, dict):
                upstream_error["detail"] = ContentRuntimeGuardService.build_guard_error(
                    guard_result=ContentGuardResult(
                        result=str(guard.get("result") or ContentGuardService.RESULT_BLOCK),
                        risk_level=str(guard.get("risk_level") or "high"),
                        categories=guard.get("categories") if isinstance(guard.get("categories"), list) else [],
                        reason=str(guard.get("reason") or "内容完整性防护命中"),
                        action=str(guard.get("action") or "block"),
                        excerpt=guard.get("excerpt") if isinstance(guard.get("excerpt"), str) else None,
                        score_delta=guard.get("score_delta") if isinstance(guard.get("score_delta"), int) else 0,
                        confidence=guard.get("confidence") if isinstance(guard.get("confidence"), (int, float)) else 0.0,
                        matched_rules=guard.get("matched_rules") if isinstance(guard.get("matched_rules"), list) else [],
                        latency_ms=guard.get("latency_ms") if isinstance(guard.get("latency_ms"), int) else None,
                        buffer_wait_ms=guard.get("buffer_wait_ms") if isinstance(guard.get("buffer_wait_ms"), int) else None,
                        final_strategy=str(guard.get("final_strategy") or "switch_provider_exhausted"),
                    ),
                    trace_id=trace_id,
                    retried=True,
                    final=True,
        ...
        if upstream_error is not None and (content_guard_exhausted or ProxyService._attempt_count(trace) <= 1):
            ...
            content_guard_log_kwargs = ProxyService._content_guard_log_kwargs_from_trace(
                trace,
                detail=detail,
                final_strategy="switch_provider_exhausted" if content_guard_exhausted else None,
            )
            trace_for_log = ProxyService._trace_with_error_response_event(
                trace,
                status_code=status_code,
                detail=detail,
                retryable=False if content_guard_exhausted else ProxyService._is_retryable_status(status_code, detail),
            )
            LogService.upsert_log(
                db,
                log_type=log_type,
        ...
        all_failed_retryable = False if content_guard_exhausted else True
        trace_for_log = ProxyService._trace_with_error_response_event(
            trace,
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=detail,
            retryable=all_failed_retryable,
        )
        LogService.upsert_log(
            db,
            log_type=log_type,
    ...
    def _build_trace_item(
        ...
        *,
        ...
        if extra:
            item.update(extra)
        error_message = ProxyService._error_message_for_log(error) if error is not None else None
        extra_payload = dict(extra or {})
        detail_payload = dict(extra_payload) if extra_payload else None
        ...
        item["payload"] = {
            ...
            "endpoint_path": extra_payload.get("endpoint_path"),
            "protocol_type": getattr(provider_model, "protocol_type", None),
            "route_round": extra_payload.get("route_round"),
            "retry_index": extra_payload.get("retry_index"),
            "capacity_lease_acquired": result not in {"capacity_limited", "capacity_unavailable"},
            "result": result,
            "status_code": status_code,
            "latency_ms": latency_ms,
            "upstream_request_id": extra_payload.get("upstream_request_id"),
            "error_code": ProxyService._error_code_from_detail(error) if error is not None else None,
            "error_message": error_message,
            "retryable": retryable,
            "retry_wait_seconds": extra_payload.get("retry_wait_seconds"),
            "retry_wait_plan_json": (
                dumps_json(extra_payload.get("retry_wait_plan") or {})
                if "retry_wait_plan" in extra_payload
                else None
            ),
            "detail_json": dumps_json(detail_payload) if detail_payload else None,
        }
        return item
    ...
    def _stream_ttfb_ms(
        *,
        ...
        if request_started_at is None:
            return max(0, int(first_chunk_latency_ms))
        first_chunk_seen_at = (
            observed_at
            if observed_at is not None
            else attempt_started_at + (max(0, int(first_chunk_latency_ms)) / 1000)
        )
        return max(0, int((first_chunk_seen_at - request_started_at) * 1000))

    @staticmethod
    def _elapsed_ms_since(started_at: float | None, *, fallback_started_at: float | None = None) -> int | None:
        base = started_at if isinstance(started_at, (int, float)) else fallback_started_at
        if not isinstance(base, (int, float)):
            return None
        return max(0, int((time.perf_counter() - float(base)) * 1000))

    @staticmethod
    def _latest_upstream_duration_ms_from_trace(trace: list[dict] | dict | None) -> int | None:
        if not isinstance(trace, list):
            return None
        for item in reversed(trace):
            if not isinstance(item, dict) or item.get("provider_id") is None:
                continue
            value = item.get("total_duration_ms")
            if value is None:
    ...
    @staticmethod
    def _serialize_response_payload_for_logging(payload: dict[str, Any], *, setting: Any) -> str:
        sanitized = ProxyService._sanitize_for_logging(
            payload,
            mask_sensitive=getattr(setting, "mask_sensitive_fields", True),
        )
        return dumps_json(sanitized)
    ...
    def _sanitize_for_logging(value: Any, *, mask_sensitive: bool) -> Any:
        if isinstance(value, dict):
            ...
            for key, item in value.items():
                lowered = key.lower()
                if mask_sensitive and ProxyService._is_sensitive_logging_key(lowered):
                    sanitized[key] = "***"
                    continue
    ...
    @staticmethod
    def _is_sensitive_logging_key(lowered_key: str) -> bool:
        if lowered_key in {"authorization", "proxy_authorization", "api_key", "apikey", "x-api-key"}:
            return True
        sensitive_fragments = (
            "secret",
            "password",
        ...
        return any(fragment in lowered_key for fragment in sensitive_fragments)
    ...
    @staticmethod
    def _extract_stream_prefetch_display_text(
        chunk: bytes,
        *,
        ...
        ProxyService._collect_stream_log_data(
            ...
            token_limit_bytes=limit_bytes,
            stream_log_fast_path=stream_log_fast_path,
        )
        parsed_text = ProxyService._finalize_text_capture(response_text_parts)
        if parsed_text:
            return parsed_text
        return chunk[:limit_bytes].decode("utf-8", errors="ignore")
    ...
    @staticmethod
    def _build_request_log_base_kwargs(
        *,
        log_type: str,
        trace_id: str | None,
        model_name: str | None,
        ...
        if provider_model is not None:
            payload["resolved_provider_model_id"] = provider_model.id
            payload.update(ProxyService._build_provider_log_kwargs(provider_model))
        return payload

    @staticmethod
    async def _upsert_live_request_log_async(
        db: Session | None,
        *,
        ...
        payload = {
            **base_log_kwargs,
            ...
            "trace": trace,
            "attempt_count": attempt_count,
            "success": request_status == LogService.REQUEST_STATUS_RESPONDING,
            "billable": False,
        ...
        if provider_model is not None:
            payload["resolved_provider_model_id"] = provider_model.id
            payload.update(ProxyService._build_provider_log_kwargs(provider_model))
        if request_status == LogService.REQUEST_STATUS_RESPONDING and base_log_kwargs.get("is_stream"):
            payload["status_code"] = LogService.STREAM_IN_PROGRESS_STATUS_CODE
        await ProxyService._run_db_write(
            LogService.upsert_log,
            db=db,
            **payload,
    ...
    def list_models(
        ...
        *,
        ...
        model_set = {
            ...
            for candidate in RoutingService.preview_candidates_sync(
                RouteRequest(
    ...
    async def _forward_response_management_request(
        *,
        ...
        db = SessionLocal()
        try:
            providers, route_candidates = ProxyService._ordered_response_management_providers(
                db,
                route_context=route_context,
                model_name=request_model_name,
                request_path=request_path,
            )
            first_candidate = route_candidates[0] if route_candidates else None
            ProxyService._append_route_decision_trace_event(
                trace,
                route_round=1,
                candidate_count=len(route_candidates),
                selected_provider_id=first_candidate.provider.id if first_candidate is not None else None,
                selected_provider_model_id=first_candidate.provider_model.id if first_candidate is not None else None,
                selected_reason=first_candidate.selection_reason if first_candidate is not None else None,
                top_candidates=RoutingService.route_candidate_trace_summary(route_candidates),
                sticky_hit=False,
            )
            if not providers:
                detail = {
        ...
        candidate_by_provider_id = {candidate.provider.id: candidate for candidate in route_candidates}

        for provider in providers:
            provider_candidate = candidate_by_provider_id.get(provider.id)
            provider_model = provider_candidate.provider_model if provider_candidate is not None else None
            started = time.perf_counter()
            try:
                async with ProviderCapacityService.async_lease(provider, is_stream=False, provider_model=provider_model):
                ...
                latency_ms = int((time.perf_counter() - started) * 1000)
                detail = {
                    **ProxyService._capacity_service_unavailable_detail(exc),
                    "provider_id": provider.id,
                    "provider_name": provider.name,
                }
                trace.append(
    ...
    def _ordered_response_management_providers(
        ...
        *,
        ...
        model_name: str | None,
        request_path: str | None,
    ) -> tuple[list[Provider], list[Any]]:
        normalized_path = request_path or ""
        route_candidates = RoutingService.preview_candidates_sync(
            RouteRequest(
                requested_model=model_name,
                selected_model=model_name,
                endpoint_path=normalized_path,
                public_endpoint_path=normalized_path,
                policy_context=route_context,
                db=db,
                capabilities=RouteCapabilitySet(
                    require_chat_completions=normalized_path.startswith("/chat/completions"),
                    require_responses=normalized_path.startswith("/responses"),
                ),
            )
        ).candidates
        ...
        return ordered, route_candidates
