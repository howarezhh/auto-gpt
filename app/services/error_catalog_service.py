...
class ErrorCatalogService:
    ...
    CATEGORY_HANDLING_STRATEGIES: dict[str, str] = {
        ...
        "authorization": "核对 API Key、用户账号、模型、端点和中转站授权关系；记录授权对象 ID 与拒绝原因。",
        "quota": "核对账户可用余额、冻结金额和计费后台任务状态；余额恢复后允许调用方重试。",
        "rate_limit": "核对 API Key、账户、全局和中转站短窗口限流计数；等待窗口恢复或调整限流配置。",
        "capacity_limited": "核对全局、账户、API Key 和中转站活跃请求/流式请求/QPS/RPM 容量；检查 Redis 租约是否及时释放。",
        "model_unavailable": "核对模型名称、模型启用状态、API Key 授权、模型映射、挂载中转站和健康检查结果。",
        "capability_not_supported": "核对请求端点、协议类型、模型能力、工具/视觉/图片生成能力和适配开关；避免把不可无损转换的请求投递到上游。",
        "route_unavailable": "核对候选筛选 trace、模型挂载、授权、健康状态、容量、熔断和协议能力，确认为什么没有可用候选。",
        "upstream_transient": "核对上游状态码、请求 ID、候选重试 trace、网络与中转站健康；保留截断摘要，避免暴露原始敏感响应。",
        ...
        "server_error": "按 trace_id 查询服务端日志、异常类型、后台任务状态、数据库/Redis 健康和最近配置变更；对外只返回安全摘要。",
    ...
    SPECS: dict[str, ErrorSpec] = {
        ...
        "responses_endpoint_not_supported": ErrorSpec(
            ...
            "当前候选中转站或模型不支持原生 Responses API。",
            404,
            "invalid_request_error",
            "capability_not_supported",
            False,
            False,
            "查看 detail.reason_details 和 detail.diagnostic_samples，改用支持 /v1/responses 的中转站或模型。",
        ),
        "chat_completions_endpoint_not_supported": ErrorSpec(
            "chat_completions_endpoint_not_supported",
            "当前候选中转站或模型不支持原生 Chat Completions API。",
            404,
            "invalid_request_error",
            "capability_not_supported",
            False,
            False,
            "查看 detail.reason_details 和 detail.diagnostic_samples，改用支持 /v1/chat/completions 的中转站或模型。",
        ),
        ...
        "no_authorized_provider": ErrorSpec(
            ...
            "当前 API Key 没有任何已授权中转站。",
            403,
            "authentication_error",
            "authorization",
            False,
            False,
            "至少为该 API Key 授权一个可用中转站。",
        ),
        ...
        "provider_capacity_exceeded": ErrorSpec(
            ...
            True,
            True,
            "等待容量释放后重试，或启用其他健康中转站参与路由。",
        ),
        "provider_capacity_unavailable": ErrorSpec(
            "provider_capacity_unavailable",
            "中转站容量状态暂时不可用，无法安全分配请求。",
            503,
            "server_error",
            "capacity_limited",
            True,
        ...
        "route_unavailable": ErrorSpec(
            ...
            True,
            True,
            "检查模型挂载、健康状态、容量限制、熔断状态和 API Key 授权。",
        ),
        "all_providers_failed": ErrorSpec(
            "all_providers_failed",
            "所有候选中转站都请求失败，已返回最后一次失败原因。",
            502,
            "server_error",
            "upstream_transient",
            True,
        ...
        "all_providers_unavailable_after_retry": ErrorSpec(
            ...
            "所有可用中转站在等待重试窗口内仍不可用，已停止内部重试。",
            503,
            "server_error",
            "route_unavailable",
            True,
            True,
            "查看 trace 中的候选排除原因，恢复容量、健康状态或授权后重试。",
        ),
        ...
        "internal_server_error": ErrorSpec(
            ...
            handling_strategy="按 trace_id 查询异常堆栈、请求摘要、依赖健康、后台任务状态和最近配置变更；禁止把堆栈、Secret、数据库连接串或上游原文返回给用户。",
            alert_level="critical",
        ),
    ...
    MESSAGE_CODE_HINTS: tuple[tuple[tuple[str, ...], str], ...] = (
        ...
        (("provider model not found", "中转站模型", "模型绑定不存在"), "provider_model_not_found"),
        (("provider not found", "中转站不存在"), "provider_not_found"),
...
ErrorCatalogService.SPECS.update(
    {
        ...
        "model_not_available": _spec(
            ...
            False,
            False,
            "检查模型是否启用、是否有健康中转站挂载，以及当前 API Key 是否有权限。",
        ),
        ...
        "chat_completion_provider_not_available": _spec(
            ...
            "Chat Completion 管理请求没有可用中转站。",
            503,
            "server_error",
            "route_unavailable",
            True,
            True,
            "检查支持 Chat Completions 管理接口的中转站健康与授权状态。",
        ),
        ...
        "moderation_provider_not_available": _spec(
            ...
            "Moderation 请求没有可用中转站。",
            503,
            "server_error",
            "route_unavailable",
            True,
            True,
            "检查支持 Moderation 的中转站健康、模型挂载和授权状态。",
        ),
        ...
        "provider_capacity_unavailable": _spec(
            ...
            public_message="系统繁忙或中转站容量暂时不可用。",
            log_message="中转站容量状态读取失败，通常与 Redis、容量快照或租约状态有关。",
        ...
        "sqlite_production_start_blocked": _spec(
            ...
            False,
            False,
            "切换到 PostgreSQL 或服务型数据库后启动。",
            public=False,
            public_code="internal_server_error",
            log_message="生产环境检测到 SQLite 数据库配置。",
            handling_strategy="迁移 DATABASE_URL 到 PostgreSQL，并执行独立迁移流程。",
            alert_level="critical",
        ),
