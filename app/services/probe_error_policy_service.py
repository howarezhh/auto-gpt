...
class ProbeErrorPolicyService:
    ...
    _POLICIES: dict[str, ProbeErrorPolicy] = {
        "probe_rate_limited": ProbeErrorPolicy(
            ...
            handling_strategy="等待限频窗口结束后再执行探针；本次结果不写入可用、协议或内容可信沉淀状态。",
            retryable=True,
            recoverable=True,
        ...
        "provider_maintenance_mode": ProbeErrorPolicy(
            ...
            error_category="probe_control",
            handling_strategy="维护模式下跳过自动探针；等待维护结束后再检测，不写入可用、协议或内容可信沉淀状态。",
            retryable=True,
            recoverable=True,
            update_allowed=False,
            state_update_policy="skip_all_state_updates",
        ...
        "endpoint_explicit_unsupported": ProbeErrorPolicy(
            ...
            handling_strategy="仅当错误明确指向端点或协议不支持时，协议检测可沉淀 unsupported；普通可用性探针只显示能力不支持。",
            retryable=False,
            recoverable=False,
            update_allowed=True,
        ...
        "content_integrity_violation": ProbeErrorPolicy(
            ...
            handling_strategy="内容防护探针可写入可信/内容完整性状态；普通可用性探针不得用可用字段替代内容风险。",
            retryable=False,
            recoverable=False,
            update_allowed=True,
        ...
        "client_cancelled": ProbeErrorPolicy(
            ...
            handling_strategy="客户端取消不计入上游失败，不更新可用、协议或内容可信沉淀状态。",
            retryable=False,
            recoverable=True,
            update_allowed=False,
            state_update_policy="skip_all_state_updates",
