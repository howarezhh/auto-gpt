...
- 日志、计费、可用、内容防护、路由、provider、模型、API Key 各有清晰边界。
- 核心服务依赖方向单向。
...
2. 提供商候选选择阶段统一判断端点协议、流式、视觉、工具调用、图片生成、容量、熔断、内容完整性、可信策略、可用、负载和路由得分。
...
8. Chat Completions / Responses 端点协议支持只能来自 provider_model 挂载级协议配置；可用性检测、端点测试、内容探针和能力探针只能记录观测结果，禁止自动改写或收紧协议支持。
...
18. 全局设置中仍存在 `route_mode/default_provider_id/manual_allow_fallback` 历史兼容面；目标架构应通过 `RouteSettingsPolicy` 把它们解释为兼容配置或迁移输入，用户可见路由主策略仍收敛为可用性优先。
...
30. 内容防护 `safe_error` 必须作为独立运行时策略进入目标架构：错误码使用 `content_integrity_safe_error`，typed event 记录命中规则和最终策略；非流式场景保留当前 Chat、Completions、Responses 三种安全兜底响应语义，`finish_reason=content_filter`，成功日志不触发 Token 补全；不能与普通 `content_integrity_violation`、候选切换失败或 provider 可用失败混为同一事件语义。
...
37. 模型查询和批测护栏必须进入模型模块策略：`GET /api/models` 当前默认分页，非分页兼容路径最多返回 500 条；模型映射列表最多 500 条；批量可用性测试最多 50 个模型目录，最大并发仍为 12；模型列表缓存 TTL 为 `0` 时表示禁用，正数最多 300 秒。目标 `ModelCatalogQueryPolicy`、`ModelMappingQueryPolicy`、`ModelHealthBatchPolicy` 必须保留这些边界，并让页面、API、测试脚本读取同一策略说明。
...
43. provider 与 provider_model 熔断状态必须由统一 `CircuitStatePolicy` 管理：`circuit_state`、`circuit_opened_at`、恢复间隔、可用恢复、内容完整性恢复、路由候选重新纳入、缓存失效和诊断文案都要从同一策略输出，避免可用、内容防护、路由和 provider 服务各自写状态。
...
50. 请求日志筛选项必须进入独立查询策略：当前 `/api/logs/filter-options` 与 `/api/user/logs/filter-options` 默认 limit 200、最大 500，`logs:filter-options:*` 缓存 10 秒，key 包含可用性检测排除、用户作用域、API Key 作用域和 limit。目标 `RequestLogFilterOptionsPolicy` 必须统一字段集合、权限作用域、TTL、写后收敛窗口和前端空状态。
...
56. 模型目录和模型映射维护需要补充策略：`ModelCatalogOptionPolicy` 保留模型选项 500 上限，`ModelHealthFilterPolicy` 保留可用筛选扫描 1000 上限，`ModelCatalogSummaryCachePolicy` 保留 `model-catalog-summary:v2` 缓存与失效，`ModelMappingMaintenancePolicy` 保留映射规范化 500 批大小。
...
61. 可用性检测扫描预算需要 `HealthScanBudgetPolicy`：自动定时扫描 provider 上限 200，手动流式和非流式全部检查单次也保留 200 上限；后台任务要记录本轮扫描范围、跳过原因、processed/success/failed、是否截断和后续多批执行方式。
...
71. 可用性检测和 provider 测试必须保留查询投影：`ProviderHealthQueryProjection` 使用 SQL 条件聚合；`ScheduledProviderLoadPolicy` 在自动 L0 连通性检查中使用 `include_models=False`；`ProviderModelDirectTestPolicy` 按 `(provider_model_id, provider_id)` 直查单模型挂载；`SelectedProviderConnectivityPolicy` 只查询选中 provider ID；`HealthStreamProgressPolicy` 保留 200 条有界进度队列并允许丢弃最旧中间进度。
...
2. 新增 `app/application/proxy_pipeline.py`
   ...
   其中 `resolve_model_mapping` 只允许返回目标模型和映射 trace；`select_route_candidates` 才允许调用提供商挂载、容量租约、可用熔断、内容完整性、可信策略和端点协议判断。
...
6. 新增 `RouteSettingsPolicy`：
   ...
   - 正式 `/v1/*` 候选选择只暴露可用性优先、容量、候选数、重试等待、可信策略等仍有效配置。
...
- 路由策略可用纯单元测试覆盖。
- 新增策略不需要改代理主链路。
- API Key 权限、容量和可用状态的职责边界更清楚。
- 路由候选构造、缓存 key、重试 trace 和容量读取都有明确预算，极端 provider/model 挂载规模下不会因为架构拆分丢失当前保护。

### 验收标准

- `RoutingEngine` 单元测试覆盖可用性优先、会话粘性、容量硬过滤、provider RPM/QPS、内容完整性过滤、候选数上限。
- `RoutingEngine` 单元测试必须覆盖“模型映射已选定目标模型后，提供商候选阶段再判断端点协议、流式、视觉、工具调用、图片生成、容量、熔断、内容完整性和可信策略”。
- 权重字段不得作为候选排序输入；排序输入应收敛为可用、当前负载、路由得分、失败率、成本、会话粘性和硬约束过滤结果。
- 内容完整性、可信等级和内容违规次数不得作为软排序加减分重复参与评分；它们只能通过候选硬过滤、诊断原因、运行时内容防护策略或事件证据表达。
- 路由单元测试必须覆盖端点协议权威来源：Chat 请求只命中挂载级配置支持 `chat_completions` 的 provider_model，Responses 请求只命中挂载级配置支持 `responses` 的 provider_model；可用性探针失败不得自动修改协议支持。
...
1. 拆分 `ProviderService`：
   ...
   - `ProviderOptionProjectionPolicy`：统一 provider 下拉、筛选器、Playground 选择器只读取 provider 摘要字段和模型名列表，不加载价格、可用、内容防护、容量等完整模型挂载字段。
...
2. 拆分 `ModelCatalogService`：
   ...
   - `ModelHealthFilterPolicy`：统一可用筛选扫描最多 1000 条目录，避免筛选项构造加载全量模型目录。
   ...
   - `ModelHealthBatchPolicy`：统一批量可用性测试最多 50 个模型目录、最大并发 12、超时和结果截断提示。
...
6. 熔断状态写入只能通过 `ProviderCircuitStatePolicy` 或可用/内容防护领域事件处理器：可用失败、内容污染、手动恢复、可信探针通过、provider_model 恢复都输出标准状态变更命令，再由 provider 仓储落库。路由服务只读取状态和恢复时间，不直接决定 provider 级状态写入。
...
- 模型选项、模型可用性筛选、模型 summary 和模型映射维护各自有策略边界，不再把所有查询/维护上限塞在 `ModelCatalogService`。
- 批量导入 100 个 provider 时只触发一次模型目录同步和一次自动授权绑定协调，避免过去逐条同步导致的数据库、缓存和 API Key 绑定放大。
- provider 与 provider_model 的熔断开启、恢复、诊断原因和缓存失效有同一套语义，可用性检测、内容防护和路由候选不再各自解释 `circuit_opened_at`。
...
- Playground provider 列表与 provider 管理主列表脱钩，内部调试页刷新时只读取调试所需字段，避免把价格、容量、探针和完整可用对象带入高频调试入口。
...
- 模型映射列表、模型可用性全量测试和模型列表缓存关闭语义均由策略对象覆盖单元测试。
- 模型选项查询超过 500 条时按策略截断并返回稳定排序；可用筛选不会扫描超过 1000 条；summary 缓存 key 使用 `model-catalog-summary:v2` 并在模型/provider 写操作后失效；模型映射规范化按 500 批执行。
...
4. 拆分 `LogService`：
   ...
   - `RequestLogFilterOptionsPolicy`：统一请求日志筛选项默认 200、最大 500、`logs:filter-options:*` 缓存 key、10 秒 TTL、可用性检测排除、用户/API Key 作用域和 distinct 字段集合。
...
12. 抽出统一 `QueueFailurePolicy` 与 `QueueObservationProjection`：
   ...
   - `SchedulerJobConcurrencyPolicy` 统一 APScheduler 定时任务 `max_instances=1`、`coalesce=True`，禁止 provider 可用、模型可用性、内容探针、Token backfill、数据保留和事件清理在错过触发后并发补跑。
...
- 请求日志筛选项不再由页面或当前页数据反推，而是由服务端按权限作用域、可用性检测排除和上限返回稳定 DTO。
...
`HealthService` 约 3682 行，并依赖代理主链路，导致可用性检测不是独立模块。
...
7. 新增可用查询投影策略：
   ...
   - `HealthCheckModelPreloadPolicy`：全部可用性检测若后续阶段会访问 provider_models，必须在查询阶段一次性预加载必要字段，禁止按 provider 懒加载形成 N+1。
   - `ManualProviderScanLimitPolicy`：手动流式和非流式全部检查保留 `MANUAL_PROVIDER_SCAN_LIMIT=200`，并在响应或后台摘要中说明是否截断。
   - `HealthStreamDisconnectPolicy`：流式可用性检测必须检测客户端断开，断开或生成器退出时取消后台任务，并释放手动检查槽位。
...
- 新增能力探针只需注册 probe。
- 自动可用性巡检与单次手动全部检查都对低配部署可控；管理员需要全量诊断时可通过多批任务完成，而不是单次无限扫描。
- 全部可用性检测不再因 provider models 懒加载产生 N+1 查询；流式检查在浏览器断开或网络慢消费时会及时取消或丢弃中间进度，不拖慢探测任务。
...
- 定时可用性检测最多扫描 200 个 provider 并写入任务摘要；手动流式和非流式全部检查单次最多扫描 200 个 provider，真正全量诊断必须通过分页、游标或后台多批任务继续。
...
- L0 自动连通性检查不得预加载 provider_models；单模型测试只能查询目标 `ProviderModel`；流式可用进度队列超过 200 条时不得无限积压，完成/错误事件必须能被客户端收到。
...
8. 新增 `ContentGuardConfigApplicationService` 或等价用例，统一保存设置、规则、URL 白名单、自动预检间隔和外部检测配置；写入后必须失效 settings、provider runtime、route candidates、v1 models、模型选项等受影响缓存。
   ...
   - `ContentGuardProbeSourcePolicy` 统一普通手动探针 `manual_probe`、可信手动探针 `manual_trust_probe`、自动预检、运行时检测和可用性探针的来源标记。
...
12. 新增 `ContentGuardActionStateMachine` 或等价策略矩阵，明确以下关系：
   ...
   - 自动可用性探针、自动预检、手动普通探针和手动可信探针必须有不同来源标记，日志、详情和状态机均能区分“人工恢复”和“真实自动恢复”。
...
14. 新增 `ContentGuardScanPolicy`：
   ...
   - 运行时检测、本地检测、可信探针、可用性探针和回归脚本必须复用同一策略。
   - 对超深 JSON、超大数组和循环风险输入返回安全截断结果，不得阻塞代理主链路。

15. 新增 `HealthContentProbeProjection`：
   - 手动模型可用性测试附带的 `content_probe_results` 必须展开为可用性探针事件或内容防护事件。
   - 可用性日志详情、可用导出、手动测试弹窗看到同一份内容探针明细。
   - 内容污染不得默认计入普通 provider 可用失败；需要单独事件分类和统计口径。
...
- 可信探针、运行时检测和规则算法可以分别测试，且三类必需探针口径不会被可用性检测或路由层重复实现。
- 配置变更、规则变更、可信状态变更和自动隔离都能触发明确缓存失效，避免路由候选、模型列表和前端展示继续读旧状态。
- 外部渠道检测具备安全边界，日志中能追踪目标但不泄露密钥或敏感 base_url。
- 内容防护事件、请求日志摘要、可用性探针事件和 provider/model 状态变化具备统一证据链，管理员能从一次污染请求追溯到命中规则、动作策略、是否切换 provider、是否更新状态、是否触发告警。
...
- provider/provider_model 内容状态恢复具备来源证据，管理员能区分手动普通检测、手动可信检测、自动预检和可用性探针触发的恢复。
...
- `/api/content-guard/trust-probe`、`/api/content-guard/precheck/probe`、路由可信筛选和可用性内容探针均复用同一探针聚合逻辑。
...
- `record_only`、关闭高风险拦截、关闭内容防护总开关三种场景必须有回归测试，证明不会误扣 provider 内容完整性分、误熔断、误写 blocked 或误计入可用失败。
- Chat 多模态 content、Responses summary/reasoning、image_url 对象、工具参数、超深 JSON 和超多节点输入必须有统一扫描测试；超出扫描预算时必须安全截断并保留可观测摘要。
- 单次 `block` 探针、连续失败 3 次、手动可信探针、自动可用性探针四类场景必须分别测试：只有状态机允许的自动处置才能写入 `blocked`、打开熔断或触发路由排除；仅记录和关闭开关场景只能写事件证据。
- 可用性检测回归必须证明手动模型测试中的内容探针会落到可查询事件，且可用性日志详情能展示固定答案、外链广告识别、流式污染检测的结果。
...
- 手动普通探针、手动可信探针、自动预检和可用性探针写入 `content_probe_results_json` 时必须带 `detection_source` 与 `manual_detection`；恢复 provider/provider_model 时同步验证 `circuit_opened_at` 清理和缓存失效。
...
7. 建立脚本级资源策略：
   - `MonotonicTimeoutPolicy`：可用等待、日志等待和服务 live 检查统一使用 `time.monotonic()`，禁止受系统时间调整影响。
