...
17. 前端重构必须把日志高级筛选/详情、provider 更多操作菜单、内容可信探针弹窗、内容防护外部密钥输入控件、benchmark 模型选择与报告展示拆成独立页面或组件模块；拆分过程中保持现有界面和操作流程不变。
...
19. `base.html` 与 `auth_base.html` 静态资源版本参数必须随 `app/static/` 可执行资源变化同步更新；当前最新版本为 `app.css?v=20260608-09`、`app.js?v=20260608-39`。
...
当前工作区已经出现 IP 管理模块的主要后端骨架：`IpManagementSetting`、`IpAccessRule`、`IpManagementEvent`、`ClientIpResolver`、`IpManagementRuleService`、`IpManagementRateLimitService`、`IpManagementEventService`、`IpManagementService`、`IpManagementMiddleware`、`/api/ip-management/*`、`source_ip_blocked/source_ip_rate_limited` 错误码和 `2026-06-08_add_ip_management_module.sql` 迁移均已存在，`main.py` 也注册了中间件与路由。

但它仍不是完整的低耦合模块：

- `IpManagementService` 同时负责配置、缓存、规则、决策、序列化、总览统计和事件写入。
- 中间件直接创建 `SessionLocal`，没有通过统一应用用例控制事务和失败策略。
- 事件写入同步 commit 到数据库，未接入统一日志队列或采样策略。
- `event_sample_rate`、`store_raw_headers_enabled`、`fail_open_enabled` 等字段尚未完全驱动运行行为。
- `/ip-management` 页面路由已存在，但当前未发现 `app/templates/ip_management.html`、导航入口和 `app.js` 页面初始化，前端管理闭环缺失。
...
4. 补齐前端页面：
   - 新增 `app/templates/ip_management.html`。
   - `base.html` 管理端左侧导航增加“IP 管理”入口。
   - `app/static/js/app.js` 增加 `initIpManagementPage` 或后续拆分为 `pages/ip_management.js`。
...
6. 错误目录和测试：
   ...
   - 回归默认关闭、未启用作用域、观察模式、阻断、限流、可信代理链、伪造 XFF 和 Forwarded IPv6。
...
- `Forwarded` 支持引号、IPv6 方括号、端口剥离和多段代理链。
...
`app/static/js/app.js` 约 15820 行，`app/static/css/app.css` 约 12480 行，所有页面逻辑集中在单文件。当前新增的日志高级筛选、类型化日志详情、provider 固定定位操作菜单、内容防护可信探针弹窗、内容防护 Tab 切换、治理列表恢复/隔离操作、探针结果自动切 Tab、外部检测密钥显示/清除控件、benchmark 模型选择器和报告工作流都继续堆在同一脚本中，前端模块化已不仅是代码整洁问题，而是后续功能可测试性问题。
...
- `base.html` 与 `auth_base.html` 静态资源版本参数已同步更新到 `app.css?v=20260608-09`、`app.js?v=20260608-39`。
...
- IP 管理页面不得把转发头解析和 API Key 白名单切换混成一个开关；可信代理解析、规则处置、事件记录和 API Key 白名单兼容切换必须分别展示。
...
11. IP 管理模块架构化，补齐页面、默认 no-op、可信代理解析和旧来源 IP 兼容策略。
12. 前端 JS/CSS 模块化。
13. 数据库迁移标准化。
