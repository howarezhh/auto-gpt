...
18. `base.html` 静态资源版本参数必须随 `app/static/` 可执行资源变化同步更新；当前最新版本为 `app.css?v=20260608-06`、`app.js?v=20260608-29`。
19. `RequestContentGuardEvent` 已经不只存在于 ORM 和请求时间线中，当前已有独立内容防护事件列表、筛选、导出入口和前端 Tab；目标日志体系必须继续把详情、CSV、请求详情摘要、内容防护页面最近事件收敛到同一 projection 和稳定 schema。
20. typed log 导出必须有稳定 schema 和表头版本；计费日志、内容防护日志、后台任务日志不能依赖当前结果集动态推导字段。
21. 后台任务观测必须区分调度任务锁事件和后台队列消费状态。`BackgroundJobEvent` 记录调度运行，队列观测记录 backlog、消费延迟、重试、死信、worker 存活和 stale running。
...
3. API Key 与用户账户余额边界：
   ...
   - 历史 `balance_amount`、`total_recharge_amount`、`token_limit_total`、`request_limit_daily`、`cost_limit_daily` 等字段必须通过迁移删除或在旧库残留检查中标记为不可用，目标 Schema、表单和账单命令禁止继续读取或写入它们。
...
- API Key 表、Schema、表单、导出和账单命令不得继续读取旧 `balance_amount`、`total_recharge_amount`、`route_mode`、`default_provider_id`、`max_candidate_count`、历史总额/日额限额字段；旧字段若在兼容迁移前残留，只能由迁移检查报告读取。
...
根目录 `问题.md` 进一步暴露了当前中间态风险：内容防护 typed event 的独立列表、日志 Tab、导出入口和正式迁移已经补齐，但事件详情、请求摘要、内容防护页面最近事件和 CSV 表头仍缺少统一投影；手动健康测试的内容探针未完整写入健康事件；SSE 探针、文本检测和 JSON 检测可能使用不同规则口径；`record_only` 或关闭高风险拦截时仍可能提前污染 provider 治理状态；规则基础校验已有改善，但规则 JSON 损坏提示、ReDoS 复杂度、跨链路 `image_url`/工具参数扫描和外部检测 API Key 生命周期仍需要治理。
...
9. 在现有外部目标 `https`、localhost/内网/IP 地址校验基础上新增 `ExternalContentProbePolicy`，继续补齐 DNS rebinding、响应大小、总超时、API Key 清理、base_url 日志脱敏和稳定外部目标 ID，禁止外部检测成为 SSRF、密钥泄露或审计不可追踪入口。
...
13. 把当前 schema 层已有的规则校验上移为 `ContentGuardRuleValidationService`：
    ...
    - 增加重复 ID、导入/重置预览、ReDoS 复杂度和文本、JSON、SSE、工具参数、`image_url` 等结构化输出字段的同一提取与扫描规则。
...
- `content_guard_required=false` 的目标语义必须清楚：它是允许 API Key 不强制可信路由，还是允许关闭运行时检测，二者只能选定一种并在前端、后端、路由缓存 key 和文档中保持一致。
...
- 外部渠道检测必须继续拒绝内网地址和非法协议，并补齐 DNS rebinding、超长响应、总超时、日志脱敏与检测完成后前端 API Key 清理。
...
- 规则编辑回归必须覆盖 `reason` 字段保存、非法 regex 拒绝、规则 JSON 损坏提示、重复 ID、ReDoS 复杂度、`image_url` 外链扫描和 URL 白名单显式域边界。
...
`app/static/js/app.js` 约 15796 行，`app/static/css/app.css` 约 12461 行，所有页面逻辑集中在单文件。当前新增的日志高级筛选、类型化日志详情、provider 固定定位操作菜单、内容防护可信探针弹窗、benchmark 模型选择器和报告工作流都继续堆在同一脚本中，前端模块化已不仅是代码整洁问题，而是后续功能可测试性问题。
