...
def test_request_logs_have_token_finalize_pending_partial_index() -> None:
    ...
    assert "billable = true" in predicate.lower()
...
def test_token_finalize_pending_index_migration_exists() -> None:
    migration = Path("migrations/2026-06-14_add_request_log_billable.sql").read_text(encoding="utf-8")
    ...
    assert "billable = TRUE" in migration
